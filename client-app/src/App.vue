...
import { AppSurface, LoadingState } from '@/components'
...
const setupStatus = computed(() => {
  return authStore.isAuthenticated && fullSetupStatus.value
    ? fullSetupStatus.value
    : publicSetupStatus.value
})
const isPublicAuthRoute = computed(() => route.meta.guest === true || route.name === 'callback')
const isAppLoading = computed(
  () => !isPublicAuthRoute.value && (isSetupLoading.value || !authStore.isInitialized),
)
...
<template>
  <div class="lp-page" :class="{ 'writing-app': showHeader }">
    <AppHeader v-if="showHeader" @sign-out="signOut" />
    ...
    >
      <AppSurface
        class="border-lp-danger/20 bg-lp-danger-soft text-sm font-semibold text-lp-danger"
      >
        {{ setupError }}
      </AppSurface>
    </main>
